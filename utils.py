...
# This file contains general utility functions for training
...
class TensorboardLogger(object):
    ...
    def update_figure(self, head, fig, step):
        self.writer.add_figure(head + "/inference", fig, self.step if step is None else step)
...
def save_model(args, output_dir, epoch, model, optimizer, model_ema=None):
...
def evaluate_binary_segmentation(output, target, threshold=0.5):
    out_probs = torch.nn.Sigmoid()(output)
    pred = (out_probs > threshold).long()
    return jaccard_score(target.cpu().flatten(), pred.cpu().flatten())