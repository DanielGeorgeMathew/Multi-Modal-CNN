...
from wound_rgb import getDataLoaders
...
# This script can be run to train RGB based model on an RGB Dataset for segmentation

def parse_args():
    parser = ArgumentParserRGBDSegmentation(
        description='RGBD Segmentation',
...
def prepareDatasetPathFiles(args):
    ...
    root_path = 'datasets/Wound-RGB/Wound-RGB/finetune/train'
    target_path = 'wound-rgb-dataset/train.txt'
    if not os.path.exists(os.path.dirname(target_path)):
        os.makedirs(os.path.dirname(target_path))

    file_paths = []
    for rgb_path in glob(os.path.join(root_path, 'imgs/*.png')):
        mask_path = rgb_path.replace('imgs', 'masks')
        if os.path.exists(rgb_path) and os.path.exists(mask_path):
            file_paths.append('{} {}\n'.format(rgb_path, mask_path))

    with open(target_path, 'w') as fp:
        fp.writelines(file_paths)

    root_path = 'datasets/Wound-RGB/Wound-RGB/finetune/val'
    target_path = 'wound-rgb-dataset/val.txt'
    if not os.path.exists(os.path.dirname(target_path)):
        os.makedirs(os.path.dirname(target_path))
...
def train_main():
    ...
    # pos_wght = torch.Tensor([81.83]).cuda()
    # Adjust this weight based on ratio of wound pixels to non wound pixels. Otherwise model will not converge and result in Nan/Inf loss
    criterion = torch.nn.BCEWithLogitsLoss(torch.Tensor([20.0]).cuda())
        ...
        if valid_loader is not None:
            ...
            with torch.no_grad():
                ...
                for data_iter_step, samples in enumerate(valid_loader):
                    ...
                    ax[1].imshow((torch.nn.Sigmoid()(output[random_index]) > 0.5).detach().cpu().numpy())
                    ax[2].imshow(targets[random_index].squeeze().detach().cpu().numpy())
                    ...
                    # ax[0].imshow(images[random_index].permute((1, 2, 0)).detach().cpu().numpy())
                    # ax[1].imshow((torch.nn.Sigmoid()(output[random_index]) > 0.5))
