...
# This file contains dataloaders for RGBT dataset
...
class woundFinetuneDataset(Dataset):
    def __init__(self, dataset_paths, transforms):
        ...
        self.thermal_paths = dataset_paths['thermal']

    def __getitem__(self, index):
        img_path = self.image_paths[index]
        mask_path = self.mask_paths[index]
        thermal_path = self.thermal_paths[index]

        img = Image.open(img_path)
        mask = Image.open(mask_path)
        mask = ImageOps.grayscale(mask)
        thermal = cv2.imread(thermal_path, 0)
        thermal = Image.fromarray(thermal)

        sample = {'image': img, 'mask': mask, 'depth': thermal}
