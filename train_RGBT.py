...
from wound_rgbt import getDataLoaders
import utils
...
# This script can be used to train an RGB-Thermal Model on an RGB-Thermal Dataset for wound binary segmentation.
...
def prepareDatasetPathFiles(args):
    ...
    root_path = 'datasets/Wound-RGBT/results'
    target_path = 'wound-rgbt-dataset/train.txt'

    file_paths = []
    for thermal_path in glob(os.path.join(root_path, 'case_*/day_*/results/scene_*/thermal.png')):
        rgb_path = thermal_path.replace('thermal', 'photo')
        mask_path = thermal_path.replace('thermal', 'mask')
        if os.path.exists(rgb_path) and os.path.exists(mask_path):
            file_paths.append('{} {} {}\n'.format(rgb_path, thermal_path, mask_path))

    with open(target_path, 'w') as fp:
        fp.writelines(file_paths)

    root_path = 'datasets/Wound-RGBT/results'
    target_path = 'wound-rgbt-dataset/val.txt'
